...
def concrete_stretched(alpha, l=0., r = 1.):
	nu = torch.zeros_like(alpha).uniform_().clamp_(0.0001, 0.9999)
	s = (torch.sigmoid(nu.log() - (1-nu).log() + alpha)).detach()
	u = s*(r-l) + l
	t = u.clamp(0, 1000)
	z = t.clamp(-1000, 1)
	...
	dz_dalpha = (t<1).float()*(u>0).float()*du_ds*(s*(1-s))
...
class Timer:
	def __init__(self):
		...
		self.counter = 0
	def start(self, n):
		self.starts[n] = time.time()
	def end(self, n):
		if not n in self.ints:
			self.ints[n] = 0
		self.ints[n] += time.time() - self.starts[n]
	def print(self):
		self.counter += 1
		for n in self.ints:
			print("[%s] %.3lf" % (n, self.ints[n]/self.counter))
...
def train(args, train_dataset, model, tokenizer):
	...
	for n,p in model.named_parameters():
		...
		if not "classifier" in n:
			all_alpha.append(bert_params[name][2].reshape(-1))
			all_add.append(bert_params[name][1].reshape(-1))
	model_device = list(model.named_parameters())[0][1].device
	print("bert_params names=", bert_params.keys())
	...
	if args.per_params_alpha == 1:
		...
		for n, p in model.named_parameters():
			...
			if not "classifier" in n:
				all_pp_alpha.append(alpha.reshape(-1))
	...
	for _ in train_iterator:
		...
		for step, batch in enumerate(epoch_iterator):
			...
			for n,p in model.named_parameters():
				if not "classifier" in n:
					tmp_z2, tmp_z2_grad = concrete_stretched(per_params_alpha[n], args.concrete_lower, args.concrete_upper)
					tmp_z, tmp_z_grad = concrete_stretched(bert_params[n][2], args.concrete_lower, args.concrete_upper)
					cmp_z.append(tmp_z.reshape(-1))
					cmp_z_grad.append(tmp_z_grad.reshape(-1))
					cmp_z2_.append(tmp_z2.reshape(-1))
					cmp_z2_grad_.append(tmp_z2_grad.reshape(-1))

					cmp_grad_params[n] = [bert_params[n][1] * tmp_z2, tmp_z * tmp_z2, tmp_z_grad, bert_params[n][1] * tmp_z]

					"""
					if torch.sum(torch.abs(z[counter:counter+tmp_z.numel()]-tmp_z.reshape(-1))) > 0:
						l = counter
						r=counter+tmp_z.numel()
						print("all_alpha=", all_alpha[l:r], " bert_params[n[2]=", bert_params[n][2])
						embed()
					"""
					counter += tmp_z.numel()
			# HACK
			cmp_z = torch.cat(cmp_z, dim=0)
			cmp_z_grad = torch.cat(cmp_z_grad, dim=0)
			cmp_z2_ = torch.cat(cmp_z2_, dim=0)
			cmp_z2_grad_ = torch.cat(cmp_z2_grad_, dim=0)

			# HACK
			z = cmp_z
			z_grad = cmp_z_grad
			z2_ = cmp_z2_
			z2_grad_ = cmp_z2_grad_
			del cmp_z, cmp_z_grad, cmp_z2_, cmp_z2_grad_

			print("z[:5]=", z[:5], " z2_[:5]=", z2_[:5])
			# expand z2_ to match z
			z2 = []
			z2_grad = []
			counter = 0
			for n, p in model.named_parameters():
				if not "classifier" in n:
					z2.append(z2_[counter:counter+1].expand(p.numel()))
					z2_grad.append(z2_grad_[counter:counter+1].expand(p.numel()))
					counter += 1
			z2 = torch.cat(z2, dim=0)
			z2_grad = torch.cat(z2_grad, dim=0)
			del z2_
			del z2_grad_

			# DEBUGDEBUG
			#print("confirm all_alpha has grad", all_alpha.grad, all_alpha.requires_grad)

			l0_pen_sum += torch.sigmoid(all_alpha - log_ratio).sum()
			l0_pen_sum += torch.sigmoid(all_pp_alpha - log_ratio).sum()
			assert all_add.size(0) == z2.size(0) and z2.size(0) == z.size(0)
			grad_0 = all_add * z2
			grad_1 = z * z2
			grad_2 = z_grad
			grad_3 = all_add * z
			...
			for n, p in model.named_parameters():
				if "classifier" in n:
					...
					next_counter = counter + p.numel()
					grad_params[n] = [grad_0[counter:next_counter].reshape_as(p), grad_1[counter:next_counter].reshape_as(p),
							grad_2[counter:next_counter].reshape_as(p), grad_3[counter:next_counter].reshape_as(p)]
					per_params_z_grad[n] = z2_grad[counter]
					
					"""
					diff = 0
					for i in range(4):
						diff += torch.sum(torch.abs(grad_params[n][i]-cmp_grad_params[n][i]))
					if diff > 0:
						embed()
					"""

					new_p = bert_params[n][0].data + grad_params[n][1].data*bert_params[n][1].data
					p.data.copy_(new_p)
					nonzero_params += (grad_params[n][1]>0).float().detach().sum().item()  # DEBUG
			...
			if (step + 1) % args.gradient_accumulation_steps == 0 or (
				...
				sum_l0_pen = l0_pen_sum
				sum_l0_pen.sum().backward()
				del z,z_grad, z2, z2_grad
				del grad_0, grad_1, grad_2, grad_3
				...
				if (step + 1)% 100 == 0:  # DEBUG
					print("empeirical prob: %.4lf" % (1.0*nonzero_params/pn4))
				...
				if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
					...
					print("NOW empeirical prob: %.4lf" % (1.0*nonzero_params/pn4))
